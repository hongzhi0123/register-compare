...
<div>
	...
	<button type="button" class="inline-flex items-center justify-center rounded-md bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700" onclick={openPicker}>
		Importer le JSON
	</button>
</div>
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
		<div class="w-full max-w-md rounded-lg bg-white p-6 shadow-2xl">
			<div class="flex items-start gap-3">
				<div class="mt-0.5 h-6 w-6 animate-spin rounded-full border-2 border-gray-300 border-t-red-600"></div>
				<div class="flex-1">
					<h3 class="text-base font-semibold text-gray-900">Import du fichier REGAFI</h3>
					<p class="mt-1 text-sm text-gray-600">{uploadStatus}</p>
					<div class="mt-4 h-2 overflow-hidden rounded-full bg-gray-200">
						{#if uploadProgress === null}
							<div class="h-full w-1/3 animate-pulse rounded-full bg-red-600"></div>
						{:else}
							<div
								class="h-full rounded-full bg-red-600 transition-[width] duration-200"
