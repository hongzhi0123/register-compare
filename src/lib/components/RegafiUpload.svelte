<script lang="ts">
	...
	async function processFile(file: File) {
		...
		try {
			const res = await fetch('/api/regafi', {
				method: 'POST',
...
<div
	...
	class:border-red-500={dragging}
	class:bg-red-50={dragging}
	class:border-gray-300={!dragging}
	class:bg-gray-50={!dragging && !fileName}
	ondragover={handleDragOver}
	ondragleave={handleDragLeave}
	ondrop={handleDrop}
	onclick={() => document.getElementById('regafi-file-input')?.click()}
	role="button"
	tabindex="0"
	onkeydown={(e) => e.key === 'Enter' && document.getElementById('regafi-file-input')?.click()}
>
	<input
	...
	{:else}
		<div class="flex flex-col items-center gap-2">
			...
			<p class="text-sm text-gray-600">
				Déposez le fichier JSON REGAFI ici ou <span class="text-red-600 font-medium">parcourez</span>
			</p>
			<p class="text-xs text-gray-400">Fichier téléchargé depuis l'API REGAFI (catalogue-banque)</p>
		</div>
	{/if}
</div>
